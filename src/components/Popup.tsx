...
import type { PopupProps } from "./ContentPage";
...
export default function Popup({ courseId, courseCategory, id }: PopupProps ) {
  const [course, setCourse] = useState<ContentProps|null>(null);

  useEffect(() => {
    if(id && courseId && courseCategory) {
      const detailInfo = serviceInfo.find((item) => item.id === id) as {
        courses: Array<{
          courseCategory?: string;
          categoryLists: ContentProps[];
        }>;
      } | undefined;

      if (detailInfo) {
        const category = detailInfo.courses.find(
          (x) => x.courseCategory === courseCategory
        );
        ...
        const foundCourse = category.categoryLists.find(
